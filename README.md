<div align="center">

<h1>Game Client Engineer · Systems & Automation</h1>

**System Architecture · Tooling · AI Integration · DevOps Pipeline**  
**Unreal Engine 5.6 · Unity · C++17 · C# · Python**

<br>

<!-- Tech Badges -->
<img src="https://img.shields.io/badge/Unreal-5.6-000?style=flat&logo=unrealengine&logoColor=white"/>
<img src="https://img.shields.io/badge/Unity-FFFFFF?style=flat&logo=unity&logoColor=000"/>
<img src="https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white"/>
<img src="https://img.shields.io/badge/C%23-239120?style=flat&logo=csharp&logoColor=white"/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/DevOps-GitHub_Actions-2088FF?style=flat&logo=githubactions&logoColor=white"/>

</div>

---

### About Me

- 게임 클라이언트 아키텍처와 개발 환경을 설계하는 엔지니어입니다.    
- UE·Unity·AI·DevOps·툴링 기반의 구조 설계를 통해  
  프로젝트의 안정성·지속성·완주 가능성을 보장합니다.
---

### Core Competencies  
- **Game Client Architecture (UE5.6, C++)** — 네트워크·스트리밍·UI·AI·서브시스템 구조 설계 및 안정화  
- **Cross-Engine Development (Unity, C#)** — 상용 개발·라이브 운영 기반 엔진 구조 최적화  
- **Tooling & Automation** — 기획·아트가 직접 활용 가능한 파이프라인·에디터 자동화 구축  
- **AI Integration** — Whisper·GPT·TTS 기반 실시간 음성 인터랙션 시스템 구현  
- **Stability & Performance** — 로딩·계측·디버깅 체계화로 프로젝트 성능·안정성 향상  
---

### Research & Development
| Preview & Title | Description | Link |
|-----------------|-------------|------|
| <div align="center"><img src="https://raw.githubusercontent.com/doppleddiggong/doppleddiggong/main/Preview/preview_yisan.png" height="140"/><br><b>AI·언리얼 음성 인터랙션</b></div> | STT → GPT → TTS 기반 **실시간 음성 인터랙션 시스템**.<br>UE5 네트워크 환경에서 안정적으로 동작하도록 **요청·응답 파이프라인 구조 설계**. | [LINK](https://github.com/doppleddiggong/YiSan) |
| <div align="center"><img src="https://raw.githubusercontent.com/doppleddiggong/doppleddiggong/main/Preview/preview_coffeetoolbar.png" height="140"/><br><b>CoffeeToolbar</b></div> | UE 에디터 내 반복 작업·레벨 관리·명령 실행을 자동화하는 **생산성 워크플로 툴링**.<br>Editor 확장 기반 **실사용 중심 기능 제공**. | [LINK](https://github.com/doppleddiggong/CoffeeToolbar) |
| <div align="center"><img src="https://raw.githubusercontent.com/doppleddiggong/doppleddiggong/main/Preview/preview_sheet2so.png" height="140"/><br><b>Sheet2SO</b></div> | Google Sheet → ScriptableObject 자동 변환 **데이터 파이프라인**.<br>기획/아트 변경 사항을 즉시 SO로 반영하여 **협업 워크플로 자동화**. | [LINK](https://github.com/doppleddiggong/Sheet2SO) |

---
### Tech Stack
<p> <img src="https://img.shields.io/badge/C++17-00599C?logo=cplusplus&logoColor=white&style=flat"/> <img src="https://img.shields.io/badge/C%23-239120?logo=csharp&logoColor=white&style=flat"/> <img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white&style=flat"/> <img src="https://img.shields.io/badge/Unreal-000?logo=unrealengine&logoColor=white&style=flat"/> <img src="https://img.shields.io/badge/Unity-FFF?logo=unity&logoColor=000&style=flat"/> <img src="https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white&style=flat"/> <img src="https://img.shields.io/badge/GitHub_Actions-2088FF?logo=githubactions&logoColor=white&style=flat"/> </p>

---

Contact: ju100.bae@gmail.com  
GitHub: https://github.com/doppleddiggong
